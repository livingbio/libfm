...
#include "src/fm_predict.h"
#include "src/fm_predict_mcmc_simultaneous.h"
...
int main(int argc, char **argv) {
	...
	try {
		...
		const std::string param_train		= cmdline.registerParameter("train", "r=regression, c=binary classification [MANDATORY]");
		const std::string param_predict		= cmdline.registerParameter("predict", "r=regression, c=binary classification [MANDATORY]");
		const std::string param_meta_file	= cmdline.registerParameter("meta", "filename for meta information about data set");
		const std::string param_train_file	= cmdline.registerParameter("train_file", "filename for training data [MANDATORY]");
		const std::string param_test_file	= cmdline.registerParameter("test_file", "filename for test data [MANDATORY]");
		...
		const std::string param_save_model_etc = cmdline.registerParameter("save_model_etc", "write model min/max label to text file; default = ''");
		const std::string param_load_model_etc = cmdline.registerParameter("load_model_etc", "load model min/max label from text file; default = ''");
		...
		if (cmdline.hasParameter(param_train)) {
			// (1) Load the data
			...
			fml->meta = &meta;
			if (! cmdline.getValue("train").compare("r") ) {
				fml->task = 0;
			} else if (! cmdline.getValue("train").compare("c") ) {
				fml->task = 1;
			...
			if (cmdline.hasParameter(param_save_model_etc)) {
				std::string filename = cmdline.getValue(param_save_model_etc);
				std::cout << "writing " << filename << std::endl; std::cout.flush();
				std::ofstream out (filename.c_str(), std::ios_base::binary);
				if (out.is_open()) {
					out << fml->min_target << std::endl;
					out << fml->max_target << std::endl;
					out.close();
				} else {
					throw "could not open " + filename;
				}
		...
		else if (cmdline.hasParameter(param_predict)){
			std::cout << "Loading test... \t" << std::endl;
			Data test(
				cmdline.getValue(param_cache_size, 0),
				! (!cmdline.getValue(param_method).compare("mcmc")), // no original data for mcmc
			...
			uint num_all_attribute = test.num_feature;
			fm_model fm;
			{
				fm.num_attribute = num_all_attribute;
				fm.init_stdev = cmdline.getValue(param_init_stdev, 0.1);
			...
			fm_predict* fmp;
			if (! cmdline.getValue(param_method).compare("mcmc")) {
	 			fmp = new fm_predict_mcmc_simultaneous();
			} else {
				throw "unknown method";
			}


			if (! cmdline.getValue("predict").compare("r") ) {
				fmp->task = 0;
			} else if (! cmdline.getValue("predict").compare("c") ) {
				fmp->task = 1;
				for (uint i = 0; i < test.target.dim; i++) { if (test.target(i) <= 0.0) { test.target(i) = -1.0; } else {test.target(i) = 1.0; } }
			} else {
				throw "unknown task";
			...
			if (cmdline.hasParameter(param_load_model_etc)) {
				std::string filename = cmdline.getValue(param_load_model_etc);
				std::cout << "reading " << filename << std::endl; std::cout.flush();
				std::ifstream in(filename.c_str(), std::ios_base::in | std::ios_base::binary);
				if (in.is_open()) {
					in >> fmp->min_target;
